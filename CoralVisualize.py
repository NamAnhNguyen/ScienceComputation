...
def DLA(radius, gif):
    ...
    while not isComplete:
        ...
        if gif:
            if randomWalkerCount in intervalSavePic:
                ...
                filename = "corals/cluster"+label+".png"
                # print(filename)
    ...
    ax.voxels(mat, facecolor='red', edgecolor='pink')
    # plt.ylabel("direction, $y$", fontsize=15)
    plt.savefig("corals/cluster.png", dpi=200)
    plt.close()
    print(usedInterval)
    if gif:
        with imageio.get_writer('corals/movie.gif', mode='I') as writer:
            for i in usedInterval:
                filename = "corals/cluster"+str(i)+".png"
                image = imageio.imread(filename)
                writer.append_data(image)
                os.remove(filename)
            image = imageio.imread("corals/cluster.png")
            writer.append_data(image)
